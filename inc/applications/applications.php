<?php

namespace Impeka\Applications;

if ( ! defined( 'ABSPATH' ) ) {
    exit;
}

require_once __DIR__ . '/classes/interface-form-submission-error.php';
require_once __DIR__ . '/classes/class-form-submission-error-base.php';
require_once __DIR__ . '/classes/class-application-type-form-builder.php';
require_once __DIR__ . '/classes/interface-application-interface.php';
require_once __DIR__ . '/classes/class-application-base.php';
require_once __DIR__ . '/classes/class-application.php';
require_once __DIR__ . '/classes/class-application-registrar.php';
require_once __DIR__ . '/classes/class-evaluation-registrar.php';
require_once __DIR__ . '/classes/class-evaluation-form-builder.php';
require_once __DIR__ . '/classes/class-evaluation.php';
require_once __DIR__ . '/classes/class-summary-form-page.php';
require_once __DIR__ . '/classes/class-email-base.php';
require_once __DIR__ . '/classes/class-application-submitted-email.php';
require_once __DIR__ . '/classes/class-evaluation-submitted-email.php';
require_once __DIR__ . '/classes/class-request-create-evaluation.php';
require_once __DIR__ . '/classes/class-evaluation-form-hooks.php';
require_once __DIR__ . '/classes/class-evaluation-template-helpers.php';
require_once __DIR__ . '/classes/class-application-template-manager.php';
require_once __DIR__ . '/classes/class-application-view-manager.php';
require_once __DIR__ . '/classes/class-application-error.php';
require_once __DIR__ . '/classes/class-request-create-application.php';
require_once __DIR__ . '/classes/class-request-update-application.php';
require_once __DIR__ . '/classes/class-application-submission-guard.php';
require_once __DIR__ . '/classes/class-settings.php';
require_once __DIR__ . '/classes/class-request-update-evaluation.php';
require_once __DIR__ . '/classes/class-evaluation-submission-guard.php';
require_once __DIR__ . '/classes/class-application-evaluations-metabox.php';
require_once __DIR__ . '/classes/class-application-access.php';
require_once __DIR__ . '/classes/class-application-form-hooks.php';
require_once __DIR__ . '/classes/class-application-template-helpers.php';
require_once __DIR__ . '/helper-functions.php';

ApplicationRegistrar::get_instance();
EvaluationRegistrar::get_instance();
EvaluationFormBuilder::get_instance();
ApplicationTypeFormBuilder::get_instance();
ApplicationViewManager::get_instance();
RequestCreateApplication::get_instance();
RequestUpdateApplication::get_instance();
RequestUpdateEvaluation::get_instance();
RequestCreateEvaluation::get_instance();
Settings::get_instance();
ApplicationEvaluationsMetabox::get_instance();
new ApplicationSubmissionGuard();
new EvaluationSubmissionGuard();
new ApplicationTemplateManager();
new ApplicationAccess();
new ApplicationFormHooks();
new EvaluationFormHooks();
